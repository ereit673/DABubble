...
export class MessageComponent implements OnInit, OnDestroy {
  ...
  toggleEmojiPicker() {
    if (this.message.docId) {
      if (
        this.emojiPickerService.isMessageEmojiPickerOpen(this.message.docId)
      ) {
  ...
  isEmojiPickerOpenForThisMessage(): boolean {
    return this.emojiPickerService.isMessageEmojiPickerOpen(
      this.message.docId ?? ''
    );
  }
  ...
  addEmojiToTextArea(emoji: string) {
    ...
    if (startPos === null || endPos === null) {
      ...
      const newText =
        this.message.message.substring(0, startPos) +
        emoji +
    ...
    this.messagesService.getMessage(message.docId).then((originalMessage) => {
      if (!originalMessage)
        return console.error(
          'Fehler: Nachricht nicht gefunden in Firestore:',
          message.docId
        );
  ...
  updateMessageText(message: Partial<Message>, originalMessage: Message) {
    ...
    if (message.docId && originalMessage.createdBy) {
      this.messagesService
        .updateMessage(message.docId, originalMessage.createdBy, updateData)
        .then()
        .catch((error) => {
  ...
  ckeckThredMessageAndWidth(message: any) {
    ...
    if (length > 0 && window.innerWidth < 400) {
      return true;
    } else return false;
  }
}
