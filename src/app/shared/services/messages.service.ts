...
export class MessagesService {
  ...
  loadMessagesForChannel(channelId: string | undefined): void {
    ...
    if (this.activeChannelId !== channelId) this.activeChannelId = channelId;
    ...
    this.unsubscribeMessages = onSnapshot(q, (snapshot) => {
      let messages = snapshot.docs.map((docSnap) => {
        ...
        return {
          ...
          creatorPhotoURL:
            data['creatorPhotoURL'] || '/assets/default-avatar.png',
  ...
  setMessageData(messages: Message[]) {
    ...
    messages.forEach((msg) => {
      const threadRef = collection(
        this.firestore,
        `messages/${msg.docId}/threadMessages`
      );
      onSnapshot(threadRef, (snapshot) => {
        let updatedThreads = snapshot.docs.map((doc) => ({
          docId: doc.id,
          ...doc.data(),
          timestamp: this.sharedService.convertTimestamp(
            doc.data()['timestamp']
          ),
        })) as ThreadMessage[];
        updatedThreads.sort(
  ...
  async getAllThreadMessages(userId: string): Promise<ThreadMessage[]> {
    ...
    for (const messageDoc of messagesSnapshot.docs) {
      ...
      const threadMessagesRef = collection(
        this.firestore,
        `messages/${messageDoc.id}/threadMessages`
      );
  ...
  public async getAllThreadMessagesForSearch(userId: string): Promise<ThreadMessage[]> {
  ...
  async updateMessage(
    ...
    try {
      await this.updateMessageData(docId, updateData, messageRef, userId);
    } catch (error) {
      throw console.error('Fehler beim Aktualisieren der Nachricht:', error);
    }
  }
  ...
  async updateThreadMessage(
    ...
    try {
      ...
      throw console.error(
        'Fehler beim Aktualisieren der Thread-Nachricht:',
        error
      );
    }
  }
   ...
   */
  async updateThreadMessageData(
    messageId: string,
    threadDocId: string,
    updateData: Partial<ThreadMessage>,
    threadMessageRef: DocumentReference,
    userId: string
  ) {
    if (updateData.reactions) {
    ...
    if (!existingReaction.userIds.includes(userId))
      ...
      if (index > -1) {
        ...
        if (existingReaction.userIds.length === 0) {
          ...
          if (reactionIndex > -1) updatedReactions.splice(reactionIndex, 1);
        }
      }
    }
  }
